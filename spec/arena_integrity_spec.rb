    ...
    context "structural violations" do
      it "detects a parent / child mismatch" do
        ...
        expect { arena.check_integrity!(root) }
          .to raise_error(Mdarena::Arena::IntegrityError, /parent mismatch/)
      end

      it "detects a broken prev_sibling chain" do
        root = arena.add_node(Mdarena::NodeType::DOCUMENT, source_start: 0, source_len: 0)
        a = arena.add_node(Mdarena::NodeType::PARAGRAPH, source_start: 0, source_len: 0)
        b = arena.add_node(Mdarena::NodeType::PARAGRAPH, source_start: 0, source_len: 0)
        ...
        expect { arena.check_integrity!(root) }
          .to raise_error(Mdarena::Arena::IntegrityError, /prev_sibling/)
      end
      ...
      it "detects first_child / last_child inconsistency" do
        ...
        expect { arena.check_integrity!(root) }
          .to raise_error(Mdarena::Arena::IntegrityError, /first_child=99 but last_child=-1/)
      end
      ...
      it "detects a shared subtree (same node reached twice)" do
        ...
        expect { arena.check_integrity!(root) }
          .to raise_error(Mdarena::Arena::IntegrityError, /reached twice/)
      end

      it "rejects a non-existent root id" do
        expect { arena.check_integrity!(42) }
          .to raise_error(Mdarena::Arena::IntegrityError, /has no row/)
      end
      ...
      it "rejects a root whose parent is not NO_NODE" do
        ...
        expect { arena.check_integrity!(root) }
          .to raise_error(Mdarena::Arena::IntegrityError, /non-NO_NODE parent/)
      end
    end
  end
end
