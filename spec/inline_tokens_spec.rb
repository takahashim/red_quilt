...
RSpec.describe Markdast::Inline::Tokens do
  let(:tokens) { described_class.new }

  describe "#emit" do
    it "appends a token and returns its id" do
      id = tokens.emit(Markdast::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 5)
      expect(id).to eq(0)
      expect(tokens.length).to eq(1)
      expect(tokens.kind(0)).to eq(Markdast::Inline::TokenKind::TEXT)
    ...
    it "stores per-token int/str payloads" do
      id = tokens.emit(
        Markdast::Inline::TokenKind::DELIM_RUN,
    ...
    it "assigns monotonically increasing ids" do
      a = tokens.emit(Markdast::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 1)
      b = tokens.emit(Markdast::Inline::TokenKind::TEXT, start_byte: 1, end_byte: 2)
  ...
  describe "#clear" do
    it "drops length to zero while remaining usable" do
      3.times { |i| tokens.emit(Markdast::Inline::TokenKind::TEXT, start_byte: i, end_byte: i + 1) }
      ...
      id = tokens.emit(Markdast::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 1)
      expect(id).to eq(0)
      expect(tokens.length).to eq(1)
    end
  end
