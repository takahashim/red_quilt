...
RSpec.describe Mdarena::Inline::Builder do
  let(:source) { "" }
  let(:arena) { Mdarena::Arena.new(source) }
  let(:references) { {} }
  ...
  def children_summary(parent_id)
    ...
    until child == -1
      out << {
        type: Mdarena::NodeType.name_for(arena.type(child)),
  ...
  def child_text_kinds(parent_id)
    ...
    arr = []
    until child == -1
      arr << Mdarena::NodeType.name_for(arena.type(child))
      child = arena.raw_next_sibling_id(child)
    end
    arr
  end
  ...
  describe "TEXT" do
    ...
    it "emits a TEXT arena node for a TEXT token" do
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 5)
    ...
    it "coalesces adjacent span-based TEXT tokens" do
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 3)
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 3, end_byte: 5)
      builder.build(paragraph_id, tokens)
      summary = children_summary(paragraph_id)
      expect(summary.size).to eq(1)
      expect(summary[0][:text]).to eq("hello")
  ...
  describe "ENTITY / ESCAPED_CHAR" do
    ...
    it "stores the decoded literal in str1 and merges with adjacent TEXT" do
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 1)
      tokens.emit(Mdarena::Inline::TokenKind::ENTITY, start_byte: 1, end_byte: 6, str1: "&")
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 6, end_byte: 7)
      builder.build(paragraph_id, tokens)
  ...
  describe "LINE_ENDING" do
    ...
    it "creates SOFTBREAK by default" do
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 1)
      tokens.emit(Mdarena::Inline::TokenKind::LINE_ENDING, start_byte: 1, end_byte: 2, int1: 0)
      tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 2, end_byte: 3)
      builder.build(paragraph_id, tokens)
    ...
    context "with two or more trailing spaces" do
      ...
      it "creates HARDBREAK and strips trailing spaces" do
        tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 3) # "a  "
        tokens.emit(Mdarena::Inline::TokenKind::LINE_ENDING, start_byte: 3, end_byte: 4, int1: 2)
        tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 4, end_byte: 5)
        builder.build(paragraph_id, tokens)
    ...
    context "with backslash-form hardbreak" do
      ...
      it "creates HARDBREAK when int2 == 1" do
        tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 0, end_byte: 1)
        tokens.emit(Mdarena::Inline::TokenKind::LINE_ENDING, start_byte: 1, end_byte: 3, int1: 0, int2: 1)
        tokens.emit(Mdarena::Inline::TokenKind::TEXT, start_byte: 3, end_byte: 4)
        builder.build(paragraph_id, tokens)
  ...
  describe "HTML_INLINE" do
    ...
    it "emits an HTML_INLINE node with the matched text in str1" do
      tokens.emit(Mdarena::Inline::TokenKind::HTML_INLINE,
  ...
  describe "AUTOLINK" do
    context "URI autolink" do
      ...
      it "emits LINK with TEXT child, str1 = destination" do
        tokens.emit(Mdarena::Inline::TokenKind::AUTOLINK_URI,
    ...
    context "email autolink" do
      ...
      it "wraps destination with mailto:" do
        tokens.emit(Mdarena::Inline::TokenKind::AUTOLINK_EMAIL,
  ...
  describe "CODE_SPAN" do
    ...
    def lex(src)
      tokens.clear
      Mdarena::Inline::Lexer.new(src).lex_into(tokens, 0, src.bytesize)
      tokens
    end
  ...
  describe "emphasis" do
    ...
    context "unmatched `*` stays plain" do
      ...
      it "leaves the asterisk as plain text content" do
        ...
        expect(arena.type(text)).to eq(Mdarena::NodeType::TEXT)
      end
    end
