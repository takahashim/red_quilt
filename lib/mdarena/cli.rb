...
module Mdarena
  # Entry point for the `mdarena` executable. Defined as a module-level
  ...
  module CLI
    ...
    def self.parse_options(argv, stderr:)
      ...
      parser = OptionParser.new do |opts|
        ...
        opts.on("-v", "--version", "Show version") do
          stderr.puts "mdarena #{Mdarena::VERSION}"
          return 0
        end
      end
    ...
    def self.read_source(argv, stdin:, stderr:)
      if argv.empty?
        ...
        unless File.file?(path)
          stderr.puts "mdarena: no such file: #{path}"
          return nil
        end
        File.read(path)
      else
    ...
    def self.write_diagnostics(diagnostics, stderr)
      if diagnostics.empty?
        stderr.puts "mdarena: no diagnostics"
        return
      end
