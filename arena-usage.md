...
`Mdarena::Arena` は mdarena の AST 本体を保持する低レベルストレージです。本書は **Arena を直接触る人** (block parser / inline builder / renderer / カスタム transformer など、`lib/mdarena` 配下のコード) 向けに、その API と前提条件を整理したものです。

> 外部 API として AST を扱うだけなら `Mdarena::Document` と `Mdarena::NodeRef` を経由するのが標準です。Arena は内部寄りのレイヤーで、`NodeRef` の実装が依存しているデータ構造そのものです。
...
doc_id = arena.add_node(Mdarena::NodeType::DOCUMENT,
                        source_start: 0, source_len: source.bytesize)
...
arena.each_child(para_id) do |child_id|
  ...
  when Mdarena::NodeType::TEXT
    output << arena.text(child_id)
  when Mdarena::NodeType::EMPHASIS
    output << "<em>"
...
arena.reparent(para_id, first, last) if first != Mdarena::Arena::NO_NODE
...
strong_id = arena.add_node(Mdarena::NodeType::STRONG,
...
first = arena.raw_first_child_id(em_id)
last  = arena.raw_last_child_id(em_id)
arena.reparent(strong_id, first, last) if first != Mdarena::Arena::NO_NODE
