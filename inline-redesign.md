...
lib/markdast/inline_pass.rb  # 既存ファイルを修正
```

`lib/markdast/inline_parser.rb` と `lib/markdast/inline_scanner.rb` は **削除** する。
...
- `spec/markdast_spec.rb`: 既存 34 examples すべて維持 (multibyte / source_location 含む)
...
2. **新規ファイル作成**: `lib/markdast/inline/` 配下に lexer / tokens / builder / flanking
3. **`InlinePass` を新構造に差し替え**: 既存 `InlineParser`/`InlineScanner` を import している箇所を新クラスに置き換え
4. **既存テスト走らせて穴を埋める**: commonmark_compat / markdast_spec を全 green に
5. **新規単体テスト追加**: lexer / builder / flanking
6. **CommonMark emphasis examples 追加**: Phase 9-B 分のテスト
7. **古いコード削除**: `lib/markdast/inline_parser.rb`, `lib/markdast/inline_scanner.rb`
